  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        const redirectTo = redirectPaths[user.department]?.[user.position] || '/ContDashboard';
        navigate(redirectTo);
      } else {
      ...
      <div className="w-full max-w-md p-8 bg-gray-800 rounded-lg shadow-lg">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full p-3 mt-5 bg-blue-500 font-poppins h-11 rounded-full  text-white font-semibold hover:bg-blue-600 transition disabled:bg-gray-500"
          >
        ...
        <div className="mt-4 flex items-center font-poppins">
          <input type="checkbox" id="remember" className="mr-2 relative bottom-9" />
          <label htmlFor="remember" className="text-gray-400 text-sm relative bottom-9">Remember me</label>
        </div>
      </div>
    </div>
  );
