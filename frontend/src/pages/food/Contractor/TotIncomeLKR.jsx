...
const TotIncomeLKR = ({ contracts }) => {
  ...
  const pendingTotal = pendingContracts.reduce((sum, contract) => {
    const cost = Number(contract.totalCostLKR || 0);
    return sum + cost;
  }, 0);
      ...
      <div className="text-center space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <p className="text-xl font-bold text-orange-500">LKR {pendingTotal.toFixed(2)}</p>
          </div>
          <div>
            <p className="text-sm text-gray-400">Approved</p>
            <p className="text-xl font-bold text-green-500">LKR {approvedTotal.toFixed(2)}</p>
          </div>
        </div>
      </div>
    </div>
